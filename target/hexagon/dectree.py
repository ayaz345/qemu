...
enc_classes = {iset.iset[tag]["enc_class"] for tag in encs}
subinsn_enc_classes = {
...
ext_enc_classes = {
    ...
    if enc_class not in ("NORMAL", "16BIT")
    and not enc_class.startswith("SUBINSN_")
}
...
def every_bit_counts(bitset):
    return all(
        len({bits[:i] + bits[i + 1 :] for bits in bitset}) != len(bitset)
...
def auto_separate(node):
    ...
    differentiator_opcode_bit = [
        ...
        and not opcode_bit_is_0_for_all[i]
        and not opcode_bit_is_1_for_all[i]
        for i in range(enc_width)
    ]
    best_width = 0
    for width in range(4, 0, -1):
        for lsb in range(enc_width - width, -1, -1):
            bitset = {encs[tag][lsb : lsb + width] for tag in tags}
...
def table_name(parents, node):
    ...
    if tag in subinsn_groupings:
        ...
        if enc_class in ext_enc_classes:
            name = f"DECODE_EXT_{enc_class}"
        elif enc_class in subinsn_enc_classes:
            name = f"DECODE_SUBINSN_{enc_class}"
        else:
            name = f"DECODE_ROOT_{enc_width}"
    if node != root:
...
def print_node(f, node, parents):
    ...
    print(
        f"DECODE_NEW_TABLE({name},{2**width},DECODE_SEPARATOR_BITS({lsb},{width}))",
        file=f,
    )
    for child in node["children"]:
        if len(child["leaves"]) == 0:
            ...
            if tag in subinsn_groupings:
                ...
                if "RESERVED" in tag:
                    ...
                    print(f'SUBINSNS({tag},{class_a},{class_b},"{enc}")', file=f)
            elif tag in iset.enc_ext_spaces:
                enc = iset.enc_ext_spaces[tag].replace(" ", "")
                print(f'EXTSPACE({tag},"{enc}")', file=f)
            else:
                enc = "".join(reversed(encs[tag]))
                print(f'TERMINAL({tag},"{enc}")', file=f)
        else:
            print(f"TABLE_LINK({table_name(parents + [node], child)})", file=f)
    print(
        f"DECODE_END_TABLE({name},{2**width},DECODE_SEPARATOR_BITS({lsb},{width}))",
        file=f,
    )
    print(file=f)
    parents.append(node)
...
def print_op_info(f):
    for tag in sorted(encs.keys(), key=iset.tags.index):
        ...
        for reg in regs:
            ...
            reg_enc_fields = re.findall(f"{reg_letter}+", enc)
        ...
        for imm in imms:
            ...
            imm_width = int(imm[1])
            imm_shift = imm[2]
            imm_shift = int(imm_shift) if imm_shift else 0
            imm_letter = "i" if imm_type.islower() else "I"
            remainder = imm_width
            for m in reversed(list(re.finditer(f"{imm_letter}+", enc))):
